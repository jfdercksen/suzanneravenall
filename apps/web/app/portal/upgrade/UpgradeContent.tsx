  ...
  return (
    <motion.div
      ...
      className={`relative flex flex-col rounded-xl p-5 transition-all duration-300 ${
        isCurrentTier
          ? 'bg-brand-primary-800 ring-2 ring-white/70 shadow-xl shadow-black/50'
          : isUpgrade
          ? 'bg-brand-primary-800 border border-white/25 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/40'
          : 'bg-brand-primary-800/40 border border-white/10 opacity-40 pointer-events-none'
      }`}
    >
      {isCurrentTier && (
        <div className="absolute -top-3 left-4">
          <span className="bg-white text-brand-primary text-[10px] font-semibold uppercase tracking-widest px-3 py-0.5 rounded-full">
            Your Plan
          </span>
        </div>
      )}
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex-1 min-w-0">
          <p className="text-[10px] font-semibold text-brand-accent-400 uppercase tracking-[0.2em] mb-0.5">
        ...
        <span className="text-[10px] text-white/60 font-mono shrink-0 mt-0.5">{tier.sku}</span>
      </div>

      <div className="mb-3">
        <span className="text-white font-semibold">{formatZAR(tier.zarPrice)}</span>
        {tier.usdPrice > 0 && (
          <span className="text-white/70 text-xs ml-1.5">${tier.usdPrice} USD · annual</span>
        )}
        {!tier.annual && tier.zarPrice === 0 && (
          <span className="text-white/70 text-xs ml-1.5">Unlimited</span>
        )}
        {tier.annual && tier.zarPrice > 0 && tier.usdPrice === 0 && (
          <span className="text-white/70 text-xs ml-1.5">per year</span>
        )}
      </div>

      <div className="flex flex-wrap gap-1 mb-4">
        {tier.canChargeClients && (
          <span className="text-[9px] px-2 py-0.5 bg-white/10 text-white/80 rounded-full font-medium">
            Can charge clients
          </span>
        )}
        {tier.requiresTraining && (
          <span className="text-[9px] px-2 py-0.5 border border-white/25 text-white/80 rounded-full font-medium">
            Requires training
          </span>
        )}
      </div>
      ...
      {isUpgrade && (
        <Link
          ...
          className="mt-auto w-full text-center py-2.5 px-4 bg-white hover:bg-brand-sand text-brand-primary font-semibold text-xs uppercase tracking-widest rounded-xl transition-colors duration-300"
        >
          Upgrade Now
        </Link>
      )}
      {isCurrentTier && (
        <div className="mt-auto w-full text-center py-2.5 px-4 bg-white/10 text-white/70 font-semibold text-xs uppercase tracking-widest rounded-xl cursor-default">
          Current Plan
        </div>
      )}
    </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <Link
              ...
              className="inline-flex items-center justify-center px-8 py-4 border border-white/40 hover:border-white/60 text-white font-semibold text-sm uppercase tracking-widest rounded-button transition-colors duration-300"
            >
              Back to Dashboard
            </Link>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div>
              <motion.h3
                ...
                className="text-xl font-semibold text-white mb-6 pb-4 border-b border-white/15"
              >
                Akashic Track
                <span className="ml-2 text-xs font-normal text-white/60 uppercase tracking-widest">S0213 – S0225</span>
              </motion.h3>
            ...
            <div>
              ...
              >
                ...
                <span className="ml-2 text-xs font-normal text-white/60 uppercase tracking-widest">S0226 – S0239</span>
              </motion.h3>
              <div className="space-y-4">
