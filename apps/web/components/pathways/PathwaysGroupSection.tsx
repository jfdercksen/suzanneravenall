  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Immersion cards: 4 items, white cards on the sand section */}
        <motion.ul
        ...
        >
          {groupPathways.map((immersion) => (
            <motion.li key={immersion.id} variants={cardVariants} className="h-full">
              <div className="group relative flex h-full flex-col overflow-hidden rounded-card bg-white border border-brand-border p-7 transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl">
                <div className="relative z-10 flex h-full flex-col">
                  <span className="inline-flex w-fit items-center rounded-button bg-brand-sand border border-brand-border px-3 py-1 text-[10px] uppercase tracking-[0.2em] font-medium text-brand-ink mb-5">
                    In Development
                  </span>
                  <h3 className="text-xl lg:text-2xl font-light text-brand-primary leading-tight mb-3">
                    {immersion.title}
                  </h3>
                  <p className="text-sm text-brand-muted font-light leading-relaxed mb-8">
