...
// Monochrome since the 15 Sep design standard: one ink (currentColor, set by
// the text-brand-primary class on the svg) at two strengths, in place of the
// old electric blue (#1719F4) and cyan. SOFT is the strength of what used to
// be the cyan tone; the glows are kept faint so they read as a grey halo.
...
const PARTICLES = [
  { x: 80, y: 60, duration: 2.4, delay: 0 },
  { x: 320, y: 50, duration: 3.1, delay: 0.3 },
  { x: 540, y: 70, duration: 2.8, delay: 0.7 },
  { x: 60, y: 330, duration: 3.4, delay: 1.1 },
  { x: 300, y: 360, duration: 2.6, delay: 1.5 },
  { x: 520, y: 350, duration: 3.0, delay: 0.2 },
  { x: 350, y: 160, duration: 2.2, delay: 0.9 },
  { x: 340, y: 250, duration: 3.6, delay: 1.3 },
  { x: 150, y: 40, duration: 2.9, delay: 0.5 },
  { x: 560, y: 250, duration: 2.5, delay: 1.7 },
]

function brainPaths(stroke: string, outerWidth: number, foldWidth: number, soft = false) {
  const outerOpacity = soft ? SOFT : 1
  return (
    <>
      ...
      />
      {/* Left folds */}
      <path d="M-55-30 C-45-40 -30-30 -35-15" stroke={INK} strokeOpacity={SOFT} strokeWidth={foldWidth} strokeLinecap="round" fill="none" />
      <path d="M-60 10 C-48 0 -32 10 -38 25" stroke={INK} strokeOpacity={SOFT} strokeWidth={foldWidth} strokeLinecap="round" fill="none" />
      <path d="M-45 40 C-35 32 -20 38 -25 52" stroke={INK} strokeOpacity={SOFT} strokeWidth={foldWidth * 0.8} strokeLinecap="round" fill="none" />
      {/* Right folds */}
      <path d="M55-30 C45-40 30-30 35-15" stroke={INK} strokeOpacity={SOFT} strokeWidth={foldWidth} strokeLinecap="round" fill="none" />
      <path d="M60 10 C48 0 32 10 38 25" stroke={INK} strokeOpacity={SOFT} strokeWidth={foldWidth} strokeLinecap="round" fill="none" />
      <path d="M45 40 C35 32 20 38 25 52" stroke={INK} strokeOpacity={SOFT} strokeWidth={foldWidth * 0.8} strokeLinecap="round" fill="none" />
      {/* Neural nodes on centre line */}
      <circle cx="0" cy="-40" r="4" fill={INK} />
...
export default function PatternDiagnosticIllustration() {
  return (
    <svg
      viewBox="0 0 600 400"
      className="w-full h-auto max-w-[600px] text-brand-primary"
      aria-hidden="true"
      focusable="false"
    >
      <defs>
        <linearGradient id="pdhBrainGradient" x1="0%" y1="0%" x2="0%" y2="100%">
          ...
          <stop offset="100%" stopColor={INK} stopOpacity={SOFT} />
        </linearGradient>
        ...
        <g key={i}>
          ...
          <g transform={`translate(${s.x} ${s.y})`}>
            ...
            >
              ...
              <g transform="scale(0.45)">{brainPaths(INK, 4.5, 4, true)}</g>
            </motion.g>
          </g>
        </g>
      ))}
